...
# #устновка напоминалок после создания
@bot.message_handler(commands=['reminder'])
def reminder_message(message):
# Запрашиваем у пользователя название напоминания и дату и время напоминания
    bot.send_message(message.chat.id, 'Выберите событие для напоминания:')
    bot.register_next_step_handler(message, datetime_reminder)
...
bot.set_my_commands([
    ...
    telebot.types.BotCommand("/reminder", 'Установить напоминание')
])
...
def get_reminder(message):
    try:
        ...
        #если дата напоминания раньше сегодняшней
            ...
            bot.send_message(message.chat.id, 'Додик, проверь время!')
            datetime_reminder(message)
        else:
