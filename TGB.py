...
def answering(message):
    ...
    if text.lower() == "войти":
        ...
        if db.execute_read_query(connection, check_login)[0][0] == 1:
            ...
            markup.add(button1, button2)
            bot.send_message(message.chat.id,
                             f"Учётная запись для пользователя {LOGIN} уже существует. Желаете продолжить с этой учётной записью или создать новую?",
                             reply_markup=markup)
...
def create_login(message):
    ...
    if len(LOGIN) <= 32:
        ...
        if message.text == "/start" or message.text == "/back":
            ...
            bot.send_message(message.chat.id, "Введите пароль (A-Z..a-z..1-9.. от 8 до 16 символов).",
                             reply_markup=None)
            bot.register_next_step_handler(message, password)
        else:
        ...
        msg = bot.send_message(message.chat.id,
                               "Длина введённого имени пользователя превышает 32 символа. Попробуйте снова.")
        bot.register_next_step_handler(msg, create_login)
...
def req(now, message):
    ...
    try:
        ...
        for event in today_events_20:
            events[5] += (event[0].strftime('%d.%m %H:%M') + "-" + event[1].strftime('%d.%m %H:%M') + " - " + event[
                2] + "\n")
    except Exception:
        ...
    n = 0
...
def my_events(message):
    global MARK_1, DATE_EVENT, RET_DATA, FLAG
    sel = message.text
...
def time_selector(message):
    global START_TIME, END_TIME, START_DATE, END_DATE, MARK_1, MARK_3, FLAG
    ...
    if message.text == "/start":
        ...
        if MARK_3 == 1:
            ...
            bot.send_message(message.chat.id, "Выберите параметр для изменения",
                             reply_markup=markup)
...
def time_save_event(message):
    global TEXT, START_TIME, END_TIME, START_DATE, END_DATE, MARK_1, MARK_3, FLAG
    TEXT = message.text
    if TEXT == "/start":
        start_message(message)
    elif TEXT == "/option":
...
def day_interval(message):
    ...
    if (num.isdigit() and int(num) > 0 and datetime.timedelta(days=int(num)) <= delta):
        ...
        FLAG = -1
        help_bot(message)
        bot.send_message(message.chat.id, "Вернёмся к нашему диалогу (укажите количество дней между повторами).",
                         reply_markup=None)
...
def get_reminder(message):
    global CHANGE_EVENT, DATE_EVENT, EVENT_ID, NAME_REM, START_TIME, END_TIME, START_DATE, END_DATE, MARK_1, MARK_3, FINISH, FLAG, TEXT
    try:
        answer = message.text
        if answer == "/start":
            start_message(message)
            ...
            FLAG = -1
            help_bot(message)
            bot.send_message(message.chat.id, "Вернёмся к нашему диалогу (введите время напоминания).",
                             reply_markup=None)
            bot.register_next_step_handler(message, get_reminder)
        elif answer == "/back":
...
def notification(message):
    ...
    if answer == "нет":
        ...
        button2 = types.KeyboardButton("Нет")
        markup.add(button1, button2)
        bot.send_message(message.chat.id, "Вернёмся к нашему диалогу (желаете получить напоминание).",
                         reply_markup=markup)
        bot.register_next_step_handler(message, notification)
    elif answer == "/back":
...
def name_independent(message):
    ...
    if CATEGORY == "срочное важное" or CATEGORY == "несрочное важное" or CATEGORY == "срочное неважное" or CATEGORY == "несрочное неважное":
        ...
        button4 = types.KeyboardButton("Несрочное неважное")
        markup.add(button1, button2, button3, button4)
        bot.send_message(message.chat.id, "Вернёмся к нашему диалогу (выберите категорию события).",
                         reply_markup=markup)
        bot.register_next_step_handler(message, name_independent)
...
def looping_date(message):
    ...
    if answer == "да":
        ...
        button2 = types.KeyboardButton("нет")
        markup.add(button1, button2)
        bot.send_message(message.chat.id, "Вернёмся к нашему диалогу (определите цикличность события)",
                         reply_markup=markup)
        bot.register_next_step_handler(message, looping_date)
    else:
        bot.send_message(message.chat.id, "Выберите или введите да/нет для продолжения")
...
def edit_events(message):
    ...
    if sel.lower() == "фиксированное":
        ...
        button3 = types.KeyboardButton("Независимое")
        markup.add(button1, button3)
        bot.send_message(message.chat.id, "Вернёмся к нашему диалогу (выберите интересующий Вас тип события)",
                         reply_markup=markup)
        bot.register_next_step_handler(message, edit_events)
    elif message.text == "/back" or message.text == "/option":
        keyboard(message)
...
def next_step_edit_dp(message):
    global DATE_EVENT, START_EDIT, END_EDIT, FLAG
    answer = message.text.lower()
    ...
    # заглушка для перезагрузки бота
        ...
        markup.add(button1, button3)
        bot.send_message(message.chat.id, "Вернёмся к нашему диалогу (укажите зависимость от времени да/нет)",
                         reply_markup=markup)
    ...
    # для отката на одно действие назад
        ...
        button3 = types.KeyboardButton("Независимое")
        markup.add(button1, button3)
        bot.send_message(message.chat.id, "Выберите интересующий Вас тип события",
                         reply_markup=markup)
        bot.register_next_step_handler(message, edit_events)
    else:
        bot.send_message(message.chat.id, "Проверьте корректность введённой команды или используйте кнопки.")
...
def take_num(message):
    global NEW_LIST, CHANGE_EVENT, EVENT_ID, FLAG
    answer = message.text
    ...
    # заглушка для перезагрузки бота
        ...
        FLAG = -1
        help_bot(message)
        bot.send_message(message.chat.id, "Вернёмся к нашему диалогу (введите номер интересующего Вас события)")
        bot.register_next_step_handler(message, take_num)
    # для отката на одно действие назад
    elif message.text == "/back":
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
...
def action(message):
    global CHANGE_EVENT, ID_USER, START_EDIT, DATE_EVENT, TEXT, EVENT_ID, FLAG
    answer = message.text.lower()
    if answer == "редактировать":
        ...
        bot.send_message(message.chat.id, "Желаете продолжить редактирование?", reply_markup=markup)
        bot.register_next_step_handler(message, resulting_action)
    elif (answer == "установить напоминание" and START_EDIT is not None and START_EDIT != 1):
        TEXT = None
        FLAG = -1
        datetime_reminder(message)
    elif message.text == "/start":
        ...
        if (START_EDIT is not None) and (START_EDIT != 1):
            ...
            button2 = types.KeyboardButton("удалить")
            markup.add(button1, button2)
        bot.send_message(message.chat.id, "Вернёмся к нашему диалогу (что Вы хотите сделать с событием?)",
                         reply_markup=markup)
        bot.register_next_step_handler(message, action)
    elif message.text == "/option":
        CHANGE_EVENT = None
...
def param_for_change(message):
    global START_EDIT, MARK_1, MARK_2, MARK_3, CHANGE_EVENT, FLAG
    answer = message.text.lower()
    if (answer == "важность" and START_EDIT == 1) or (answer == "важность" and START_EDIT is None):
        ...
        FLAG = 1
        depending_from_date(message)
    elif (answer == "дата и время" and START_EDIT is not None) and (answer == "дата и время" and START_EDIT != 1):
        MARK_1 = None
        MARK_3 = 1
        FLAG = None
        depending_from_time(message)
    # заглушка для перезагрузки бота
        ...
        if START_EDIT == 1:
            ...
            button2 = types.KeyboardButton("дата и время")
            markup.add(button2, button1)
        bot.send_message(message.chat.id, "Вернёмся к нашему диалогу (выберите параметр для изменения)",
                         reply_markup=markup)
        bot.register_next_step_handler(message, param_for_change)
    elif message.text == "/option":
        CHANGE_EVENT = None
...
def new_priority(message):
    global EVENT_ID, FLAG, CHANGE_EVENT
    ...
    # заглушка для перезагрузки бота
        ...
        button4 = types.KeyboardButton("Несрочное неважное")
        markup.add(button1, button2, button3, button4)
        bot.send_message(message.chat.id, "Вернёмся к нашему диалогу (введите новую степень важности)",
                         reply_markup=markup)
        bot.register_next_step_handler(message, new_priority)
    # для отката на одно действие назад
    elif message.text == "/option":
...
def new_name(message):
    global ID_USER, START_EDIT, CHANGE_EVENT, FLAG
    name = message.text
    if message.text == "/start":
        start_message(message)
    elif message.text == "/option":
        ...
        FLAG = -1
        help_bot(message)
        bot.send_message(message.chat.id, "Вернёмся к нашему диалогу (введите новое название событие)")
        bot.register_next_step_handler(message, new_name)
    elif START_EDIT is None:
        get_name = f"""
        SELECT Event
...
def resulting_action(message):
    global START_EDIT, FLAG, CHANGE_EVENT, ACTION
    answer = message.text.lower()
    if answer == "да":
...
def send_message(message):
    global FLAG, START_TIME, END_TIME, START_DATE, END_DATE, MARK_1, MARK_3, CHAT_ID, MARK_2
    if FLAG is None and message.text == "/back" and MARK_1 is None and MARK_3 == 1:
        ...
        bot.send_message(message.chat.id, "Выберите параметр для изменения", reply_markup=markup)
        bot.register_next_step_handler(message, param_for_change)
    elif FLAG is None and message.text == "/back":
        START_TIME, END_TIME, START_DATE, END_DATE, MARK_1, MARK_3 = None, None, None, None, None, None
        bot.delete_message(chat_id=CHAT_ID[0], message_id=CHAT_ID[1])
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        button1 = types.KeyboardButton("Дата и время")
        ...
        bot.send_message(message.chat.id,
                         "Выберите нужный тип события.", reply_markup=markup)
        bot.register_next_step_handler(message, type_of_event)
    elif FLAG == 1 and message.text == "/back" and MARK_1 == 1 and MARK_2 == 1:
        FLAG = None
        bot.delete_message(chat_id=CHAT_ID[0], message_id=CHAT_ID[1])
        if START_EDIT == 1:
            markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        ...
        bot.send_message(message.chat.id, "Выберите интересующий вас отрезок времени.", reply_markup=markup)
        bot.register_next_step_handler(message, free_windows)
    elif FLAG == 1 and message.text == "/back" and MARK_1 == 4:
        FLAG = None
        bot.delete_message(chat_id=CHAT_ID[0], message_id=CHAT_ID[1])
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
        button1 = types.KeyboardButton("Фиксированное")
